...
pattern = item [which, why, steps, claims, budgets, floors, estimate]  -- before outcomes are read
  ...
  -- a floor names its author: the user's mandate, a validity or execution condition, or the lead's own screen; the lead's own is a proposal until the principal takes it, and a diagnostic rejects nothing
  -- an owner child takes a disjoint question with its own budget, claims and trial slice; the trial registry, the holdout and the shared execution contract stay with the parent
...
reduce = numbers `then` senior  -- the same battery columns per era, beside pattern's floors; a parameter the source does not state comes out, or the item says why it stays
  -- the summary is a view over the full battery, never its replacement: every candidate, era and cost case, and what is unavailable named with its reason
...
judge = Accept | Fail step why | Stop why
  ...
  -- a close names the floor that failed and whose it was: the lead's screen is not the user's mandate
  -- a changed policy is a new policy with its provenance; a sealed verdict keeps the floors it was judged under
  -- a close carries the verdict and the next falsifier; that is the next run's memory

-- who
pattern  -- parent by default, memory first; a planning child spawns nothing; an owner child is the process of its slice; the parent judges the cold read, seal and battery
...
class Meter   where battery  -- comparable columns per era, with the declared null and uncertainty; units, denominator and sampling frequency named, since a borrowed column label does not carry its measurement
